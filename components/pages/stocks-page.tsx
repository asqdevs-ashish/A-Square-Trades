      ...
      <main className="container mx-auto px-4 py-8">
        ...
        >
          ...
          <p className="mt-2 text-muted-foreground">
            Trade shares of the world&apos;s leading companies
          </p>
        </motion.div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filteredAssets.map((asset, index) => (
              <StockCard key={asset.id} asset={asset} index={index} />
            ))}
          </div>
        )}
...
function StockCard({ asset, index }: StockCardProps) {
  ...
  return (
    ...
    >
      <Link href={`/?asset=${asset.id}&market=stocks`}>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-[var(--neon-purple)]/20 to-[var(--neon-cyan)]/20">
